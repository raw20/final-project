...
function NoticeTableSearchBar({ posts, copyPosts, setPosts }) {
	...
	function onSearch(e) {
		...
		if (searchValue === null || searchValue === ' ') {
			...
			const filterData = posts.filter(ele => ele.category.includes(searchValue));
			setPosts(filterData);
			...
			const filterData = posts.filter(
				ele => ele.title.includes(searchValue) || ele.category.includes(searchValue),
			);
			setPosts(filterData);
		}
	}
				...
				<div className={display ? 'contents-type' : 'contents-type on'} onClick={handlerShowType}>
					...
					<ul className={display ? 'type-tab' : 'type-tab on'}>
						{selectTypeArray.map((li, index) => (
							<li className="type-tab-list" key={index} onClick={handlerSelectType}>
								{li.name}
							</li>
						))}
					</ul>
...
export default NoticeTableSearchBar;
